<div id="inner">
  <aside>
    <nav><span class="active">pre-installation</span> <span> license </span>
      <span> configuration</span><span>completed</span></nav>
  </aside>
  <div class="middle"></div>
  <main>
    <h3>1. Server Configuration</h3>
    <p
      class="info">If any of these items are highlighted in red then please take actions to correct them. Failure to do so could lead to your installation not functioning correctly. </p>
    <table class="data">
      <thead>
      <tr>
        <th>PHP Settings</th>
      ...
      <tr>
        ...
        <td>8.1+</td>
        <td><?php echo (phpversion() >= '8.1') ? '<img src="images/yes.svg" alt="Good" class="image" />' : '<img src="images/no.svg" class="image" alt="Bad" />'; ?></td>
      </tr>
      <tr>
    ...
    <p
      class="info">These settings are recommended for PHP in order to ensure full compatibility with Car Dealer Pro.
      However, Car Dealer Pro will still operate if your settings do not quite match the recommended.</p>
    <table class="data">
      <thead>
      <tr>
        <th>Extension</th>
    ...
    <p
      class="info">In order for Car Dealer Pro to function correctly it needs to be able to access or write to certain files or directories. If you see "Unwritable" you need to change the permissions on the file or directory to allow Car Dealer Pro to write to it. </p>
    <div class="divider"></div>
    <table class="data">
        <?php getWritableCell('lib'); ?>
        <?php getWritableCell('uploads'); ?>
        <?php getWritableCell('view' . CMS_DS . 'admin' . CMS_DS . 'cache'); ?>
        <?php getWritableCell('view' . CMS_DS . 'front' . CMS_DS . 'themes' . CMS_DS . 'master' . CMS_DS . 'cache'); ?>
    </table>
  </main>
</div>
<footer><small>current <b>Car Dealer Pro v.3.0</b></small>
  <div id="buttons">
    <button type="button" class="button" onclick="document.location.href='setup.php';" name="check">Check</button>
    <button type="button" class="button" onclick="document.location.href='setup.php?step=1';" name="next"
      tabindex="3">Next
    </button>
  </div>
</footer>