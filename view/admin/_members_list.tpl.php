<?php
   ...
   * @copyright 2020
   * @version $Id: _members_list.tpl.php, v1.00 2020-03-05 10:12:05 gewa Exp $
   */
  if (!defined("_WOJO"))
...
<div class="row gutters align middle">
  ...
  <div class="columns auto phone-100">
    ...
    <a href="<?php echo Url::url("/admin/members", "new");?>" class="wojo small secondary button"><i class="icon plus alt"></i><?php echo Lang::$word->ACC_ADD;?></a>
    <?php endif;?>
...
<div class="wojo form">
  <div class="vertical margin center aligned">
    <div class="wojo divided horizontal link list">
      ...
      <a href="<?php echo Url::url(Router::$path, "?order=lname|DESC");?>" class="item<?php echo Url::setActive("order", "lname");?>">
      ...
      <a href="<?php echo Url::url(Router::$path, "?order=listings|DESC");?>" class="item<?php echo Url::setActive("order", "listings");?>">
      <?php echo Lang::$word->LISTINGS;?>
      </a>
      <a href="<?php echo Url::sortItems(Url::url(Router::$path), "order");?>" class="item"><i class="icon caret <?php echo Url::ascDesc("order");?> link"></i></a>
    </div>
  </div>
...
<table class="wojo fitted segment responsive table">
  ...
  <tbody>
    ...
    <tr id="item_<?php echo $row->id;?>">
      ...
      <td><a href="<?php echo Url::url("/admin/mailer", "?mailid=" . $row->email . "&clients=true");?>"><?php echo $row->email;?></a></td>
      ...
      <td class="auto"><?php if(Auth::hasPrivileges('manage_upay')):?>
        <a href="<?php echo Url::url("/admin/members/payments", $row->id);?>" class="wojo small primary inverted icon button"><i class="icon collection"></i></a>
        <?php endif;?>
        <a href="<?php echo Url::url("/admin/members/activity", $row->id);?>" class="wojo small secondary inverted icon button"><i class="icon person lines"></i></a>
        <?php if(Auth::hasPrivileges('edit_members')):?>
        <a href="<?php echo Url::url("/admin/members/edit", $row->id);?>" class="wojo small positive inverted icon button"><i class="icon pen alt"></i></a>
        <?php endif;?>
        <?php if(Auth::hasPrivileges('delete_members')):?>
        <a data-set='{"option":[{"trash": "trashUser","title": "<?php echo Validator::sanitize($row->fullname, "chars");?>","id":<?php echo $row->id;?>}],"action":"trash","subtext":"<?php echo Lang::$word->DELCONFIRM3;?>", "parent":"#item_<?php echo $row->id;?>"}' class="wojo small negative inverted icon button data">
